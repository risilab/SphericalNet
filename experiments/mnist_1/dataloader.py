...
class MNISTData(Dataset):
    ...
    def __init__(self, mode=TRAIN,
        ...
        self.coefs = data_process.precomputing_coefs_new(lmax=lmax, rotate=rotate, split=split, **kwargs)
        self.labels = data_process.get_labels(split=split)
        if mode == TEST:
    ...
    def __getitem__(self, idx):
        ...
        for l in range(self.lmax):
            parts.append(torch.Tensor(x[:, l**2:(l+1)**2,:].reshape(-1, 2)).unsqueeze(dim = 1))
