...
class Spherical_CNN_Classification(torch.nn.Module):
    def __init__(self, num_classes, num_layers, input_tau, hidden_tau, maxl, diag_cg = True, has_normalization = False, device = 'cuda'):
        super(Spherical_CNN_Classification, self).__init__()
        ...
        # Create learnable weights for hidden to output
        self.W_output = nn.ParameterList()
        for l in range(len(tau)):
            if tau[l] > 0:
                weight = nn.Parameter(torch.randn(tau[l], 1, 2).to(device = self.device))
                self.W_output.append(weight)

                # Parameter register
                self.register_parameter(
                        name = 'W_output_l_' + str(l),
                        param = weight
                )
                self.num_learnable_params += 1
            else:
        ...
        self.top_layer_2 = nn.Linear(256, self.num_classes).to(device = self.device)
    ...
    def forward(self, inputs):
        ...
        for l in range(len(self.W_input)):
            first = torch.view_as_complex(inputs.parts[l])
            second = torch.view_as_complex(self.W_input[l])
            hidden.append(torch.view_as_real(torch.matmul(first, second)))
        ...
        for layer in range(self.num_layers):
            # CG product
            if self.diag_cg == False:
                product = gelib.CGproduct(hidden, hidden)
            else:
                product = gelib.DiagCGproduct(hidden, hidden)
        ...
        # Weighted sum of all fragments
        outputs = []
        for l in range(len(self.W_output)):
            first = torch.view_as_complex(hidden.parts[l])
            second = torch.view_as_complex(self.W_output[l])
            outputs.append(torch.view_as_real(torch.matmul(first, second)))
        
        # Concatenate 
        outputs = torch.cat(outputs, dim = 1).squeeze(dim = 2)

        # Take the absolute value out of the complex numbers
        outputs = torch.abs(torch.view_as_complex(outputs))

        # Multilayer Perceptron for the top layer
        outputs = torch.tanh(self.top_layer_1(outputs))

        return torch.nn.functional.softmax(self.top_layer_2(outputs), dim = 1)
