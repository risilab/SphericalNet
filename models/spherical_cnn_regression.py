...
class Spherical_CNN_Regression(torch.nn.Module):
    def __init__(self, num_layers, input_tau, hidden_tau, maxl, diag_cg = True, has_normalization = True, device = 'cuda'):
        ...
        for layer in range(num_layers):
            if layer == num_layers - 1:
                if self.diag_cg == False:
                    next_tau = CGproductType(tau, tau, 0)
                else:
                    next_tau = DiagCGproductType(tau, tau, 0)
            else:
                # If this is the last layer (top of the network) then we only take the scalar out
                if self.diag_cg == False:
        ...
        for l in range(len(tau)):
            total_size += (2 * l + 1)
        self.W_top = nn.Parameter(torch.randn(total_size, 1, 2).to(device = self.device))
    ...
    def forward(self, inputs):
        ...
        second = torch.view_as_complex(self.W_top)
        outputs = torch.view_as_real(torch.matmul(first, second))
